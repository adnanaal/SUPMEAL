...
import { CreateRecipeData, MealType } from '@/types';

interface CreateRecipeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCreate: (data: CreateRecipeData & { addToMealPlanning?: boolean; plannedDate?: string; plannedMealType?: MealType }) => Promise<void>;
}
...
export function CreateRecipeModal({ isOpen, onClose, onCreate }: CreateRecipeModalProps) {
  ...
  const [addToMealPlanning, setAddToMealPlanning] = useState(false);
  const [plannedDate, setPlannedDate] = useState('');
  const [plannedMealType, setPlannedMealType] = useState<MealType | undefined>();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const submissionData = {
        ...formData,
        imagePath: imagePreview || formData.imagePath, // Use the uploaded image preview if available
        ...(addToMealPlanning && {
          addToMealPlanning: true,
          plannedDate,
          plannedMealType,
        }),
      };
      ...
      <div className="relative bg-white rounded-xl shadow-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto p-8 my-4">
        ...
        <div className="flex items-center justify-between mb-6 sticky top-0 bg-white pb-4 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-semibold text-gray-900">Create Recipe Manually</h2>
          </div>
          <button
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <label
                    htmlFor="imageUpload"
                    className="flex items-center justify-center space-x-2 px-4 py-6 border-2 border-dashed border-gray-300 rounded-lg hover:border-orange-500 hover:bg-orange-50 transition-colors cursor-pointer"
                  >
                    <Plus className="w-5 h-5 text-gray-500" />
                    <span className="text-gray-700">Upload image from PC</span>
                  </label>
                  {imagePreview && (
          ...
          <div className="space-y-4 p-4 bg-orange-50 rounded-lg">
            <div className="flex items-center space-x-3">
              <input
                type="checkbox"
                id="addToMealPlanning"
                checked={addToMealPlanning}
                onChange={(e) => setAddToMealPlanning(e.target.checked)}
                className="w-5 h-5 text-orange-500 rounded focus:ring-orange-500"
                disabled={isCreating}
              />
              <label htmlFor="addToMealPlanning" className="flex items-center space-x-2 text-gray-900 font-medium">
                <Calendar className="w-5 h-5 text-orange-600" />
                <span>Add to Meal Planning</span>
              </label>
            </div>
            ...
            {addToMealPlanning && (
              <div className="grid grid-cols-2 gap-4 mt-4">
                ...
                <div>
                  <label htmlFor="plannedMealType" className="block text-sm font-medium text-gray-700 mb-2">
                    Meal Type
                  </label>
                  <select
                    id="plannedMealType"
